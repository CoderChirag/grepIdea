...
function connectDB() {
  mongoose.connect(
    `mongodb+srv://${process.env.MONGO_USER_NAME}:${process.env.MONGO_PASSWORD}@cluster0.eeauv.mongodb.net/grepIdeaDb?retryWrites=true&w=majority`,
...
app.get("/", (req, res) => {
  ...
  if (req.isAuthenticated()) {
    ...
    res.send(
      `${html[0]}${markup}${html[1]}window.__ROUTE_DATA__ = ${serialize(data)}${
        html[2]
      }`
    );
  }
    ...
    Topic.findOne({ _id: "604367458343222b04502786" }, (err, result) => {
      if (err) {
        ...
        if (result.topics.includes(req.params.topicName)) {
          res.send(
            `${html[0]}${markup}${html[1]}window.__ROUTE_DATA__ = ${serialize(
              data
            )}${html[2]}`
          );
        } else {
...
app.post("/signup", (req, res) => {
  ...
  User.exists({ username: req.body.username }, (err, result) => {
    if (err) {
      console.log(
        `Checking of DB for checking if User Already Exists Error: \n\t${err}`
      );
...
app.post("/verify", (req, res) => {
  const enteredOTP =
    parseInt(req.body.first) * 1000 +
    parseInt(req.body.second) * 100 +
    parseInt(req.body.third) * 10 +
  ...
  User.register(
    {
      ...
      smallImgUri: data.userInfo.smallImgUri,
      imgUri: data.userInfo.imgUri,
      largeImgUri: data.userInfo.largeImgUri,
      username: req.body.username,
    },
    req.body.password,
    (err, user) => {
    ...
    if (err) {
      ...
      if (!result) {
        ...
        req.login(user, (err) => {
          if (err) {
            console.log(
              `Login with req.login method of Passport Error: \n\t${err}`
            );
            res.redirect("/500");
          } else {
            passport.authenticate("local")(req, res, () => {
      ...
      if (result) {
        Message.findOneAndUpdate(
          ...
          (error) => {
            if (error) {
              console.log(
                `Data Updation Error During Create New Msg: ${error}`
              );
              res.redirect("/error");
            } else {
        ...
        Message.create(
          { topic: req.body.topicName, msgs: { ...req.body } },
          (error, id) => {
            if (error) {
              console.log(
                `Data Creation Error During Create New Msg: ${error}`
              );
              res.redirect("/error");
            } else {
              res.redirect(`/sendMsg`);
    ...
    if (err) {
      ...
      if (result.topics.includes(req.body.value.toLowerCase().trim())) {
        ...
        Topic.findOneAndUpdate(
          { _id: "604367458343222b04502786" },
          { $push: { topics: req.body.value.toLowerCase().trim() } },
          (err, result) => {
            if (err) {
